...
class API:
    ...
    def get_beatmaps(self, since=None, mode=Modes.OSU, limit=500):
        ...
        params.update({"m": mode.value, "limit": limit})
    ...
    def get_user(self, name: str, mode=Modes.OSU):
        params = dict(k=self.token)
        params.update({"m": mode.value, "u": name})
    ...
    def get_scores(self, beatmap_id: int, username=None, mode=Modes.OSU, limit=50):
        ...
        params.update({"b": beatmap_id, "m": mode.value, "limit": limit})
        r = requests.get(self.base_url + '/api/get_user', params=params)
        try:
    ...
    def get_match(self, match_id: int):
        params = dict(k=self.token, mp=match_id)
        r = requests.get(self.base_url + '/api/get_user_recent', params=params)
        try:
            return multiplayer.MultiplayerMatch(data=r.json())
        except IndexError:
            return None
